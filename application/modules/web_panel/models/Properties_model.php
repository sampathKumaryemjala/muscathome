    ...
    function get_properties($document = array()) { //pre($document); die;
        ...
        if ($properties) {
            foreach ($properties as $property) {
                ...
                if(!empty($property['asigned_agent_id'])){
                    $property_agent_id = $property['asigned_agent_id'];
                } else {
                    $property_agent_id = $property['fk_agent_id'];
                }
                $agent_detail = $this->get_property_agent_details(["id" => $property_agent_id]);
...
//        pre($types); die;
    ...
    function property_type_count_buy() { //pre($document);
        $p_types = $this->db->get('property_types')->result_array();
        foreach ($p_types as $p_type) {
            $tttl = 0;
            $this->db->select('count(id) as ttl');
    ...
    function property_type_count_rent() { //pre($document);
        $p_types = $this->db->get('property_types')->result_array();
        foreach ($p_types as $p_type) {
            $tttl = 0;
            $this->db->select('count(id) as ttl');
