...
//        }
        ...
        $data['properties'] = $this->Properties_model->get_properties($filter);
        //unset($data['properties'][31]);
        //pre($data['properties'][31]); die;
        $data['property_type_count'] = $this->Properties_model->property_type_count_buy();
...
//        }
        ...
        $data['property_type_count'] = $this->Properties_model->property_type_count_rent();
        $data['type_count']['buy'] = $this->Properties_model->buy_count();
        $data['type_count']['rent'] = $this->Properties_model->rent_count();
        $data['featured'] = $this->Properties_model->featured_property();
    ...
    public function fav_to_property_ajax() {
        ...
        if ($this->input->post('is_fav') == 0) {
            $fav = $this->Properties_model->fav_to_property($inputs);
            //Get user detail for notification //
            $this->db->where('id',$inputs['fk_property_id']);
            $property = $this->db->get('properties')->row_array();
            $sender = $this->User_model->get_user(array('id'=>$this->session->userdata('active_user_data')['id']));
            //Notification for property agent//
            if($fav == true){
                $data['fk_user_id'] = $property['fk_agent_id'];
                $data['sender_id'] = $sender['id'];
                $data['fk_property_id'] = $inputs['fk_property_id'];
                $data['notification_type'] = 3;
                $data['description'] = $data['title'] = $sender['name']." is favorite our property";
                $this->Notifications_model->create_notification($data);
            }
